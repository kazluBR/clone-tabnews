...
describe("POST /api/v1/users", () => {
  describe("Anonymous user", () => {
    test("With unique and valid data", async () => {
      ...
      const userInDatabase = await user.findOneByUsername("kazluBR")
    ...
    test("With duplicated `email` by case", async () => {
    ...
    test("With duplicated `email` by dots", async () => {
      const response1 = await fetch(`${webserver.origin}/api/v1/users`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      const response2 = await fetch(`${webserver.origin}/api/v1/users`, {
        ...
        body: JSON.stringify({
          username: "emailduplicado4",
          email: "email.duplicado2@curso.dev",
          password: "senha123",
        }),
      })
  ...
  describe("Default user", () => {
    test("With unique and valid data", async () => {
      ...
      expect(user1Response.status).toBe(403)
